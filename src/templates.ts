  ...
  html`
    ...
    <html lang="en-US">
      <head>
        ...
        ${extra_css
          ? html`<link rel="stylesheet" href="/css/${extra_css}" />`
          : ""}
      </head>
      ...
      <body>
        ...
        <footer>
          <p>
            ...
            <a href="mailto:abhinavramesh03@gmail.com">
...
export const post_list = (posts: Post[]): HtmlString => {
  const list_items = posts.map(
    (post) =>
      html` <li>
        <h2>
          ${time(post.date, "meta")} <a href="${post.path}">${post.title}</a>
        </h2>
      </li>`
  );
...
export function time(date: Date, cls?: string): HtmlString {
  ...
  return html`<time ${cls ? `class="${cls}"` : ""} datetime="${machine}"
    >${human}</time
  >`;
}
...
export function html(strings: ArrayLike<string>, ...values: any[]): HtmlString {
